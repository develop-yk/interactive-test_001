...
| **形状ボタンに 2 秒乗せる** | **滞留選択（dwell）。ピンチが苦手でも決定できる** | — |
...
ピンチが安定しない環境向けに、**カーソルを 2 秒乗せ続けるだけで決定**する経路も用意しました。

- 対象は `data-dwell` 属性を持つ要素だけ（現状は形状選択ボタンの 3 つ）
- 進捗はボタン下端のゲージで見える（`--dwell` を毎フレーム JS が更新）
- カーソルが外れるとリセット。選択済みのボタンでは進まない
- **ピンチ中は進めない** — 3D を回している最中に勝手に決定しないため
