...
| **ボタン・スライダーに 2 秒乗せる** | **滞留選択（dwell）。ピンチが苦手でも操作できる** | — |
...
| | 内容 |
|---|---|
| 左カラム 上 | Pinch and Drag（Spin speed / Vertices / Zoom）— すべて 3D に効く |
...
左カラム `Pinch and Drag` の 3 つは、すべてこの 3D を操作します。

| コントロール | 効果 |
|---|---|
| Spin speed | 放置時の自動回転スピード。**0 で完全停止**、1.0 で 0.014 rad/frame |
| Vertices | 頂点の白い点を描くか（辺だけの表示にできる） |
| Zoom (2 hands) | 両手の間隔でオブジェクトの大きさ |

> 以前この 2 つは `Intensity` / `Wireframe` という名前のプレースホルダーで、
> 実際には何も動いていませんでした（CSS 変数を書くだけ／誰も見ないクラスを付けるだけ）。
> 3D を追加したタイミングで実機能に繋ぎ、名前も実態に合わせています。
...
```jsonc
{
  ...
  "ui":      { "shape": "dodeca", "rotation": { "x": -18, "y": 143 },
               "spin": 0.8, "vertices": true, "zoom": 1.4, "likes": 3 },
...
ジェスチャー UI で一番つまずくのが「押した瞬間にカーソルがずれて狙いを外す」問題です。
3 つ対策を入れてあります。

1. **カーソルは常に人差し指の先を使う。**
   ピンチ中だけ「親指と人差し指の中点」に切り替えると理屈の上では自然ですが、
   指を閉じた瞬間にカーソルが親指側へ飛びます。`EXP` で 1.9 倍しているので
   画面上では 40〜50px の跳びになり、60px のボタンから簡単に外れます。
2. **狙いを latch する（`js/ui.js` の `_aim`）。**
   手が開いている間（`pinchAmount > pinchOff`）に hover していた要素を覚えておき、
   ピンチ確定時はその要素を押します。押下位置で判定し直さないので、
   ずれた先が「隣のボタンの上」でも意図した方が選ばれます。
   手を離して別の場所へ動かせば latch も追従するため、古い対象が残ることはありません。
3. **ピンチした手が主導権を取る（`js/main.js`）。**
   以前は右手固定だったので、両手が映っている状態で左手をつまんでも無反応でした。

押したことが分かるよう、対象を一瞬スケールさせる `.pressed` アニメーションも入れています。

### 3 通りの入力

同じウィジェットを **ピンチ / 滞留 / マウス** のどれでも操作できます。展示では手だけ、
登壇や検証ではマウス、ピンチが安定しない人は滞留、と使い分けられます。

| ウィジェット | ピンチ | 滞留 2 秒 | マウス・キーボード |
|---|---|---|---|
| 形状ボタン ×3 | 押して決定 | 乗せ続けて決定 | クリック |
| Spin speed | ピンチしたままドラッグ | 掴んで横移動（後述） | ドラッグ / クリック / ←→ キー |
| Vertices | 押して反転 | 乗せ続けて反転 | クリック / Space |
| 3D の回転 | ピンチしたままドラッグ | — | — |

#### 滞留選択（dwell）

- 対象は `data-dwell` を持つ要素（形状ボタン 3 つ、Spin speed、Vertices）
- 進捗はゲージで見える（形状ボタンは下端のバー、細いスライダー／トグルは広がるリング）
- カーソルが外れるとリセット。選択済みのボタンでは進まない
- **ピンチしたままでも進みます**。3D の回転ドラッグ中はそもそも滞留判定に到達しない
  （ドラッグ分岐で early return する）ので、回している最中の誤爆は起きません
- 一度決めた要素は、カーソルが離れるまで再発火しません（トグルの連打防止）
- 時間は `js/ui.js` の `DWELL_MS`（既定 2000）

滞留時間は**推論フレームの時刻**（`HandState.t`）で数えています。
`performance.now()` を UI 側で直接読むと、描画が詰まった端末で滞留時間が伸び縮みします。

#### 滞留でスライダーを掴む（スティッキードラッグ）

スライダーだけは「決定」では終わらないので、滞留すると**掴んだ状態**になります。
ピンチしていない＝離上イベントが来ないため、終了条件を別に用意しています。

- 掴んだ後は、カーソルの横位置がそのまま値になる
- **スライダーの帯から縦に 90px 以上離れる**か、**ピンチする**と手放す
  （`STICKY_EXIT_PX` で調整）
- 掴んでいる間はスライダーの枠が緑になる

#### マウス・キーボード

`js/ui.js` の `enableMouse()` でまとめて配線しています。ジェスチャー側の `_drag` には
触れないので、両者が競合することはありません。スライダーは Pointer Capture を使うので、
ドラッグ中にカーソルが枠の外へ出ても追従します。
